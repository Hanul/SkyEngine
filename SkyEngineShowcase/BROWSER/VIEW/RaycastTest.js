	...
	init : (inner) => {
		...
		points.forEach((point) => {
			...
			if (Math.abs(point.x - torch.getX()) < Math.abs(point.y - torch.getY())) {
				point.x = ((point.y - torch.getY() < 0 ? -1280 : 1280) - torch.getY()) / (point.y - torch.getY()) * (point.x - torch.getX()) + torch.getX();
				point.y = point.y - torch.getY() < 0 ? -1280 : 1280;
			} else {
				point.y = (point.y - torch.getY()) / (point.x - torch.getX()) * ((point.x - torch.getX() < 0 ? -1280 : 1280) - torch.getX()) + torch.getY();
				point.x = point.x - torch.getX() < 0 ? -1280 : 1280;
			}
		...
		// 빛 생성
		let light = SkyEngine.Polygon({
			points : foundPoints,
			color : SkyEngine.Gradient({
				type : 'radial',
				startRadius : 0,
				endRadius : 500,
				colors : ['#FFCC00', 'rgba(0, 0, 0, 0)']
			}),
			zIndex : -1
		}).appendTo(SkyEngine.Screen);
